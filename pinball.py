...
def add_powerup_collision_handler(space):#collision between ball and powerup
    def remove_pow(arbiter, space, data):
        ...
        if (circ.color == THECOLORS["blue"] or circ.color == THECOLORS["red"]):#makes ball go faster
            print("fast")
        ...
        if (circ.color == THECOLORS["red"]):#adds new ball in screen and makes ball go faster
            print("both")
...
def respawn_powerup(color,space):
    start_ticks = pygame.time.get_ticks()
    if(color==THECOLORS["blue"]):
        total =random.randint(10,20)
        pos = (random.randint(400, 500), random.randint(80, 150))
    elif(color==THECOLORS["yellow"]):
        total = random.randint(20, 30)
        pos = (150, random.randint(100, 600))
    else:
        total=random.randint(30,40)
        pos =(random.randint(250, 400), random.randint(500, 700))
    while True:
        if(pygame.time.get_ticks()-start_ticks>total):
            add_powerup(space,color,pos)
def add_transport(space,posStart,posEnd,posStart2,posEnd2): #adds segments that transport ball across the layout
    body = pymunk.Body(body_type=pymunk.Body.STATIC)
    trans = pymunk.Segment(body, posStart, posEnd, 5)     #segment on left
...
def gen_rail(space,pInit,pFin):#generates series of short line segments to give impression of a curve
    ...
    for num in range(tot):
        body = pymunk.Body(body_type=pymunk.Body.STATIC)
        rail.append(pymunk.Segment(body, (pX,pY),(pX+(distX/tot),pY+(distY/tot)+((tot/2)-num)*3),6))
        pX+=distX/tot
        pY+=distY/tot
        pY+=((tot/2)-num)*3
    for line in rail:
...
def setup_level(space):
    ...
    add_transport(space, (50, 300), (50, 350), (550, 300), (550, 350))  # adds transport segments that move ball
